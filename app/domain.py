...
class PermissionDeniedError(Exception):
    def __init__(self, username: str, required_permission: str):
        ...
        super().__init__(
            f"El usuario '{username}' no tiene el permiso requerido: '{required_permission}'"
        )
...
class UnforgivableSpellError(Exception):
    def __init__(self, spell_name: str, username: str):
        ...
        self.username = username
        super().__init__(
            f"¡Uso ilegal de Hechizo Imperdonable '{spell_name}' detectado por {username}!"
        )
...
class AvadaKedavra(Hechizo):
    ...
    def execute(self, user: User, **kwargs) -> str:
        if user.level != "Ministro":
            print(f"LÓGICA DE NEGOCIO: ¡ACCESO DENEGADO! {user.username} (Rol: {user.level}) intentó lanzar Avada Kedavra.")
            raise UnforgivableSpellError(spell_name="Avada Kedavra", username=user.username)

        print(f"LÓGICA DE NEGOCIO: {user.username} (Rol: Ministro) ha ejecutado Avada Kedavra (Autorización de Admin).")
